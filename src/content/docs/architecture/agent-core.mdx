...
For complex tasks, the [Orchestrator](/orchestrator/overview/) wraps Agent Core with a Planner-Worker-Judge loop. The Planner assesses whether a task needs multi-agent coordination. Simple tasks go directly to `Core.Run()` as described here. Complex tasks are decomposed into steps, executed by concurrent workers (each using `Core.Run()`), and evaluated by a Judge. See the [Orchestrator Overview](/orchestrator/overview/) for the full flow.
</Aside>
...
The prompt is assembled by `PromptBuilder` in `internal/agent/prompt_builder.go`. The prompt now contains two sections — skills are no longer injected at build time:

```
┌────────────────────────────────┐
│ <memory>                       │  ← Task-aware context (cached per user+task)
...
**Memory context** is assembled by `GetContextForUserWithTask(userId, task)` — it scores stored conversation entries against the task's keywords and topics, returning the most relevant history alongside the user profile. Because retrieval is task-aware, the memory section changes with each distinct task. AgentLoop's internal `PromptCache` (keyed by `userId + task hash`, TTL 5 min) avoids redundant assembly when the same user repeats an identical task.

**Skill selection** happens at runtime, not at prompt-build time. When the main agent needs specialized guidance, it calls the `Find_skill` tool with a natural-language query. A lightweight **SkillAgent** subprocess evaluates the query against the skill catalog (name, description, tags for all registered skills) and returns the single best match. The selected skill's full details (instructions, sibling files, directory path) are then available to the main agent. See the [Skills Guide](/guides/skills/) for details.
