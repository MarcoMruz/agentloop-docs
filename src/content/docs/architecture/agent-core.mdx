...
The prompt is assembled by `PromptBuilder` in `internal/agent/prompt_builder.go`. Sections are ordered so the most cacheable content comes first:
...
│ </memory>                      │
│                                │
│ <skill name="git-helper">      │  ← Matched per-task via keyword detection
│   Skill instructions...        │
│ </skill>                       │
│                                │
...
**Skill detection** runs before prompt assembly via `DetectSkills(task)`. This is a simple keyword scan — no LLM call. Every available skill has a `triggers` list; if any trigger word appears in the lowercase task text, the skill is included. Matched skills are injected as `<skill name="...">` blocks.

```go
// DetectSkills — keyword matching, no LLM
