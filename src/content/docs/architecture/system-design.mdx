...
| [Skills Registry](/guides/skills/) | `internal/skills` | Skill catalog and LLM-driven selection via SkillAgent |
...
server/handler.go → session.Manager.StartSession()
    ...
    │      ├─ Build prompt: <memory>...</memory> + task text (skills loaded on demand via Find_skill)
