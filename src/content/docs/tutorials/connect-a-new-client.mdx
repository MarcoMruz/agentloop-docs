...
- **Notifications:** Have a `method` field but no `id`, server → client only
...
def handle_hitl(msg, params):
    ...
    print(f"\n⚠️  HITL: {params['toolName']} — {params['details']}")
...
<Aside type="tip">
  For production clients, add connection retry logic, proper error handling, and concurrent event processing. See the [Slack Bridge source](https://github.com/MarcoMruz/agentloop-slack) for a complete TypeScript client implementation.
</Aside>
