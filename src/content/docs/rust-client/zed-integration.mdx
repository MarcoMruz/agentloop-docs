...
The AgentLoop Rust client provides first-class integration with Zed editor through the **Adaptive Code Provider (ACP)** system. This enables seamless AI assistance directly within your editor with full access to AgentLoop's memory management, HITL approval, and session handling.
...
graph TD
    ...
    subgraph "agentloop-bridge"
        Adapter["ZedACPAdapter"]
        Client["AgentLoopClient"]
    end

    subgraph "AgentLoop Server"
    ...
    ACP --> Adapter
    Adapter --> Client
    Client -->|"Unix Socket<br/>JSON-RPC 2.0"| Memory
    Memory --> Agent
    Agent --> HITL
...
- **AgentLoop Server**: Running Go or Rust implementation
- **Feature Flag**: Enable `zed-acp` feature
