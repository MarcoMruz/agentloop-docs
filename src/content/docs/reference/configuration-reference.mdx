...
| `memory.embedding_dims` | `int` | `0` | Vector embedding dimension; `0` disables vector search (keyword-only) |
| `memory.agent.enabled` | `bool` | `false` | Enable background pi subprocess for ACE delta extraction and embeddings |
| `memory.agent.binary` | `string` | `pi` | Path to pi binary for the background memory agent |
| `memory.agent.provider` | `string` | `anthropic` | LLM provider for the background memory agent |
| `memory.agent.model` | `string` | `claude-haiku-4-5-20251001` | Model for background memory operations (use a cheap/fast model) |

## Sessions

| YAML Path | Go Type | Default | Description |
|-----------|---------|---------|-------------|
| `sessions.max_concurrent` | `int` | `5` | Max total active sessions across all users |
| `sessions.max_per_user` | `int` | `5` | Max active sessions per user |
...
| `sessions.evict_lru` | `bool` | `true` | When at capacity, abort the session with oldest last response instead of rejecting new requests |

## HITL

| YAML Path | Go Type | Default | Description |
|-----------|---------|---------|-------------|
| `hitl.always_pause_tools` | `[]string` | `[docker, git push, rm -r, curl, wget, bash, write, edit]` | Tools that always require approval |
...
| `hitl.force_hitl_keywords` | `[]string` | `[sudo, chmod, chown, systemctl, npm install, pip install, yarn add]` | Bash keywords that trigger `force-hitl.ts` approval gate |
...
sessions:
  ...
  evict_lru: true
hitl:
  always_pause_tools: [docker, git push, rm -r, curl, wget, bash, write, edit]
  timeout_seconds: 300
  timeout_action: deny
  force_hitl_keywords: [sudo, chmod, chown, systemctl, npm install, pip install, yarn add]
security:
...
<Aside type="tip">
The `orchestrator:` configuration block (agent models, worker pool size, iteration limits, and summary capping) is documented in full on the [Orchestrator Configuration](/orchestrator/configuration/) page.
</Aside>
